...
2. `New repository secret` をクリックし、以下の4つを追加：
...
| `FTP_REMOTE_DIR` | アップロード先ディレクトリ | 通常は `public_html` または `www`（サーバー設定による） |

**重要**: ロリポップのFTPは、ログイン直後のディレクトリに直接ファイルをアップロードできません。`FTP_REMOTE_DIR`に正しいディレクトリ名を設定してください。
